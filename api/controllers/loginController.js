...
const authenticateUser = async (accessKey, password) => {
    ...
    return new Promise((resolve, reject) => {
        try {
            const getAdminProfileByAccessKeyQuery = `
                SELECT * FROM admin_i WHERE admin_accessKey = ?
            `;
            // NOTE: new query for the new database - AL
            // const getAdminProfileByAccessKeyQuery = `
            //     SELECT * FROM admin_profile WHERE access_key = ?
            // `;
            db.query(getAdminProfileByAccessKeyQuery, [hashedAccesskey], async (err, result) => {
                if (err) {
